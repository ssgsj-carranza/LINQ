...
namespace DatabaseFirstLINQ
{
    class Problems
    {
        ...
        private void ProblemFour()
        {
            ...
            var names = _context.Products.Where(w => w.Name.Contains("s"));
        ...
        private void ProblemSix()
        {
            ...
            // Then print each user's email and registration date to the console.
            var regUser = _context.Users.Where(u => u.RegistrationDate.Value.Year > 2016 && u.RegistrationDate.Value.Year < 2018);
            foreach (User user in regUser)
            {
                Console.WriteLine($"Email: {user.Email} Registration Date: {user.RegistrationDate}");
            }
        ...
        private void ProblemEight()
        {
            ...
            var products = _context.ShoppingCarts.Include(p => p.Product).Where(u => u.User.Email == "afton@gmail.com");
        ...
        private void ProblemTwelve()
        {
            ...
            {
                Name= "Yugioh cards",
                Description= "Will send you to the shadow realm if you loose",
                Price= 50
            };
        ...
        private void ProblemFourteen()
        {
            ...
            var productId = _context.Products.Where(p => p.Name == "Yugioh cards").Select(r => r.Id).SingleOrDefault();
            var userId = _context.Users.Where(u => u.Email == "david@gmail.com").Select(u => u.Id).SingleOrDefault();
        ...
        private void BonusOne()
        {
           ...
           var checkUser = _context.Users.Where(u => u.Email == email && u.Password == password).SingleOrDefault();
            if (checkUser == null)
            {
                Console.WriteLine("Please enter valid email or password");
            }
            else
            {
        ...
        private void BonusTwo()
        {
            ...
            var customerUsers = _context.UserRoles.Include(ur => ur.Role).Include(ur => ur.User).Where(ur => ur.Role.RoleName != null);
            var products = _context.ShoppingCarts.Include(p => p.Product).Include(p => p.User).Where(u => customerUsers.Any(em => em.UserId == u.UserId)).ToList();
            decimal total = 0;
            foreach (ShoppingCart shoppingCart in products)
            {
                var prices = _context.ShoppingCarts.Include(p => p.Product).Where(u => u.User.Email == shoppingCart.User.Email).Select(sl => sl.Product.Price).Sum();
                Console.WriteLine($"Product Name: {shoppingCart.Product.Name} Price: {shoppingCart.Product.Price} Quantity: {shoppingCart.Quantity} Email:{shoppingCart.User.Email}");
                total = prices + total;
            }
